...
app.get("/api/fuel-price", (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Errore lettura prezzi carburante." });
  }
...
app.get("/api/names", (req, res) => {
  try {
    const stmt = db.prepare(
      "SELECT DISTINCT name FROM weekly_records ORDER BY name ASC",
    );
...
app.post("/api/records", async (req, res) => {
  const { name, weekIdentifier, kilometers, fuelEfficiency, fuelType } =
    req.body;
  if (
    !name ||
    !weekIdentifier ||
  ...
  const fuelTypeCapitalized =
    fuelType.charAt(0).toUpperCase() + fuelType.slice(1).toLowerCase();

  const existing = db
    .prepare(
      "SELECT * FROM weekly_records WHERE name = ? AND week_identifier = ?",
    )
  ...
  if (existing) {
    ...
    return res.json({
      message: "Record aggiornato con successo (KM sommati)!",
    });
  }
...
app.put("/api/records/:id", async (req, res) => {
  ...
  const existingRecord = db
    .prepare("SELECT * FROM weekly_records WHERE id = ?")
    .get(id);
  if (!existingRecord) {
    return res
      .status(404)
      .json({ message: "Record da modificare non trovato." });
  }
  ...
  if (targetRecord) {
    ...
    return res.json({
      message:
        "Record unificato con record esistente: KM sommati e record originale rimosso.",
    });
  }
...
app.delete("/api/records/:id", (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Errore durante l'eliminazione del record." });
  }
});
...
app.delete("/api/records", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Errore durante l'eliminazione dei record." });
  }
});
